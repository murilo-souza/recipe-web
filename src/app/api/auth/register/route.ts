...
import type { RegisterRequest, AuthResponse } from '@/lib/types/api';
...
export async function POST(req: Request) {
  ...
  const apiRes = await fetch(`${process.env.API_URL}/api/auth/register`, {
    method: 'POST',
  ...
  if (!apiRes.ok) {
    ...
    return Response.json({ error: errorText || 'Não foi possível registrar.' }, { status: apiRes.status });
  }
