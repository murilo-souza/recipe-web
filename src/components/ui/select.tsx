    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            'relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-xl border border-zinc-700/50 bg-zinc-800 text-zinc-200 shadow-xl ring-0 shadow-black/30 duration-100',
            'data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 data-[align-trigger=true]:animate-none',
            className,
          )}
          {...props}
        >
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn('px-2.5 py-1.5 text-xs font-medium text-zinc-500', className)}
      {...props}
    />
  );
}
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        'text-zinc-300 hover:bg-zinc-700/60 hover:text-white focus:bg-zinc-700/60 focus:text-white',
        'data-selected:bg-indigo-500/15 data-selected:text-indigo-300',
...
function SelectSeparator({ className, ...props }: SelectPrimitive.Separator.Props) {
  return (
    <SelectPrimitive.Separator
      ...
      className={cn('pointer-events-none -mx-1 my-1 h-px bg-zinc-700/50', className)}
      {...props}
    />
  );
}
...
function SelectScrollUpButton({
  ...
  return (
    <SelectPrimitive.ScrollUpArrow
      ...
      className={cn(
        "top-0 z-10 flex w-full cursor-default items-center justify-center bg-zinc-800 py-1 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
...
function SelectScrollDownButton({
  ...
  return (
    <SelectPrimitive.ScrollDownArrow
      ...
      className={cn(
        "bottom-0 z-10 flex w-full cursor-default items-center justify-center bg-zinc-800 py-1 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
